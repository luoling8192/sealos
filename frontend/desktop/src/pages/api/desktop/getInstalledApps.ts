...
import type { CRDMeta, TAppCRList, TAppConfig } from '@/types';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!payload)
      return jsonRes(res, { code: 401, message: 'token is invaild' });
    const _kc = await getUserKubeconfigNotPatch(payload.userCrName);
    if (!_kc)
      return jsonRes(res, { code: 404, message: 'user is not found' });
    ...
    const apps = [...defaultArr, ...userArr].filter(item => item.displayType !== 'hidden');
